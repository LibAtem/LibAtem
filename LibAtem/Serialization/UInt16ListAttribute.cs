...
namespace LibAtem.Serialization
{
    public class UInt16ListAttribute : SerializableAttributeBase, IRandomGeneratorAttribute
    {
        ...
        private readonly UInt16Attribute _innerAttr;

        public UInt16ListAttribute(int count)
        {
            Count = count;
            _innerAttr = new UInt16Attribute();
        }
        ...
        public override void Serialize(bool reverseBytes, byte[] data, uint start, object val)
        {
            ...
            {
                _innerAttr.Serialize(reverseBytes, data, (uint)(start + 2 * i), vals[i]);
            }
        }
        ...
        public override object Deserialize(bool reverseBytes, byte[] data, uint start, PropertyInfo prop)
        {
            ...
            {
                object v = _innerAttr.Deserialize(reverseBytes, data, (uint)(start + 2 * i), prop);
                vals.Add((uint)v);
            }
